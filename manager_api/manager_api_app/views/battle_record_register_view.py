...
class BattleRecordRegisterView(APIView):
    ...
    def post(self, request):
        ...
        BattleRecordSerializer.validate_choice_my_pokemon_blank(self, my_choice_pokemon_list)
        # リクエストデータ(選択ポケモン)を必須データシリアライズ
        BattleRecordSerializer.validate_choice_enemy_pokemon_blank(self, request.data.get('enemy_pokemon'))
        # リクエストデータ(選択ポケモン)を3匹以下にシリアライズ
        BattleRecordSerializer.validate_choice_three_lte_my_pokemon(self, my_choice_pokemon_list)
