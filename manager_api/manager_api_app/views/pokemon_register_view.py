...
from manager_api_app.serializers.pokemon_register_serializer import PokemonSerializer
...
class PokemonRegisterView(APIView):
    ...
    def post(self, request) -> list[str]:
        ...
        result_data["specialAttackEffort"] = request.data.get("specialAttackEffort")
        result_data["specialDefenceEffort"] = request.data.get("specialDefenceEffort")
        result_data["speedEffort"] = request.data.get("speedEffort")
        
        # リクエストデータをシリアライズ
        serializer_class = PokemonSerializer.validate_pokename(self, result_data["pokemonName"])
